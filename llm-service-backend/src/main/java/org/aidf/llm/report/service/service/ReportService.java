...
public class ReportService {
     ...
     * <p>
     * This method creates a {@link org.aidf.llm.report.service.model.entity.ReportTask}
     ...
     * <p>
     * The actual LLM report generation is handled asynchronously based on this task.
     *
     * @param reportRequest the request DTO containing the user's input text
     * @return a response DTO containing the task id, current status, and creation time
     */
     ...
     * @return a response DTO containing the task id, status, creation time, and report result
     * @throws RuntimeException if the task with the given ID is not found
     */
    public ReportResponse checkResponse(long taskId) {
        // check if task is found
        ReportTask task = reportTaskRepository.findById(taskId).orElseThrow(() -> new RuntimeException("Task not found"));
     ...
     * <p>
     * This method is executed asynchronously to avoid blocking the HTTP request thread.
